<template>
  <div>
    ...
    <!-- 搜索框 -->
    <van-search v-if="isShowSearch" v-model="searchKey" shape="round" background="#4fc08d" placeholder="请输入搜索关键词"
      @click="goSearch" @search="onSearch" @cancel="onCancel" />
    ...
    <van-grid :column-num="5">
      <van-grid-item v-for="(item, index) in navList" :key="item.imgUrl" :icon="item.imgUrl" :text="item.text"
        @click="gridClick(item, index)" />
    </van-grid>

    <!-- 主会场 -->
    <div class="main">
      <img src="@/assets/img/shopping/main.png" alt="">
    </div>
